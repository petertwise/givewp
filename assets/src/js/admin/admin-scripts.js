	...
	const setup_chosen_give_selects = function() {
		...
		if ( $give_chosen_containers.hasClass( 'give-chosen-settings' ) ) {
			...
			$give_chosen_containers.on( 'chosen:no_results', function() {
				let $container = $( this ).next( '.chosen-container' ),
				...
				$( '.give-select-chosen .chosen-search input' ).each( function() {
					let type = $( this ).parent().parent().parent().prev( 'select.give-select-chosen' ).data( 'search-type' );
					let placeholder = '';
	...
	const Give_Settings = {
		...
		toggle_options: function() {
			...
			emailAccess.on( 'change', function() {
				const fieldValueEmail = $( 'input[name="email_access"]:checked', '.give-setting-tab-body-general' ).val();
				const fieldValueRecaptcha = $( 'input[name="enable_recaptcha"]:checked', '.give-setting-tab-body-general' ).val();
			...
			form_featured_image.on( 'change', function() {
				const field_value = $( 'input[name="form_featured_img"]:checked', '.give-setting-tab-body-display' ).val();
			...
			sequential_ordering.on( 'change', function() {
				const field_value = $( 'input[name="sequential-ordering_status"]:checked', '.give-setting-tab-body-general' ).val(),
	...
	const GiveDonor = {
		...
		removeUser: function() {
			$( 'body' ).on( 'click', '#disconnect-donor', function( e ) {
				...
				const postData = {
					action: 'give_disconnect_donor_user',
					donor_id: donorID,
					security: $( '#edit-donor-info #_wpnonce' ).val(),
				};
										...
										if ( $fields.length ) {
											$fields.each( function() {
												...
												$.each( this.attributes, function( index, element ) {
													let old_class_name_prefix = this.value.replace( /\[/g, '_' ).replace( /]/g, '' ),
...
function give_render_responsive_tabs() {
	...
	const refactor_tabs = new Promise(
